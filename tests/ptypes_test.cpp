...
BOOST_AUTO_TEST_CASE (ptypes_min_max_test)
{
	...
	BOOST_CHECK (P_MININT64 == (pint64) 0x8000000000000000);
	BOOST_CHECK (P_MAXINT64 == (pint64) 0x7FFFFFFFFFFFFFFF);
	BOOST_CHECK (P_MAXUINT64 == (puint64) 0xFFFFFFFFFFFFFFFFU);
...
BOOST_AUTO_TEST_CASE (ptypes_host_network_test)
{
	...
	if (P_BYTE_ORDER == P_LITTLE_ENDIAN) {
		...
		pint64 pint64_val = PINT64_TO_BE (0xFFFFFFFFFFFFFF80);
		BOOST_CHECK (pint64_val == (pint64) 0x80FFFFFFFFFFFFFF);
		BOOST_CHECK (PINT64_FROM_BE (pint64_val) == (pint64) 0xFFFFFFFFFFFFFF80);
		BOOST_CHECK (PINT64_TO_LE (pint64_val) == (pint64) 0x80FFFFFFFFFFFFFF);
		BOOST_CHECK (PINT64_FROM_LE (pint64_val) == (pint64) 0x80FFFFFFFFFFFFFF);

		puint64 puint64_val = PUINT64_TO_BE (0x0000000000000080U);
		BOOST_CHECK (puint64_val == (puint64) 0x8000000000000000U);
		BOOST_CHECK (PUINT64_FROM_BE (puint64_val) == (puint64) 0x0000000000000080U);
		BOOST_CHECK (PUINT64_TO_LE (puint64_val) == (puint64) 0x8000000000000000U);
		BOOST_CHECK (PUINT64_FROM_LE (puint64_val) == (puint64) 0x8000000000000000U);
		...
		if (PLIB_SIZEOF_LONG == 8) {
			plong plong_val = PLONG_TO_BE (0xFFFFFFFFFFFFF800);
			BOOST_CHECK (plong_val == (plong) 0x00F8FFFFFFFFFFFF);
			BOOST_CHECK (PLONG_FROM_BE (plong_val) == (plong) 0xFFFFFFFFFFFFF800);
			BOOST_CHECK (PLONG_TO_LE (plong_val) == (plong) 0x00F8FFFFFFFFFFFF);
			BOOST_CHECK (PLONG_FROM_LE (plong_val) == (plong) 0x00F8FFFFFFFFFFFF);

			pulong pulong_val = PULONG_TO_BE (0x0000000000000800U);
			BOOST_CHECK (pulong_val == (pulong) 0x0008000000000000U);
			BOOST_CHECK (PULONG_FROM_BE (pulong_val) == (pulong) 0x0000000000000800U);
			BOOST_CHECK (PULONG_TO_LE (pulong_val) == (pulong) 0x0008000000000000U);
			BOOST_CHECK (PULONG_FROM_LE (pulong_val) == (pulong) 0x0008000000000000U);
		} else {
		...
		if (PLIB_SIZEOF_SIZE_T == 8) {
			psize psize_val = PSIZE_TO_BE (0x0000000000001000U);
			BOOST_CHECK (psize_val == (psize) 0x0010000000000000U);
			BOOST_CHECK (PSIZE_FROM_BE (psize_val) == (psize) 0x0000000000001000U);
			BOOST_CHECK (PSIZE_TO_LE (psize_val) == (psize) 0x0010000000000000U);
			BOOST_CHECK (PSIZE_FROM_LE (psize_val) == (psize) 0x0010000000000000U);

			pssize pssize_val = PSSIZE_TO_BE (0x000000000000F000);
			BOOST_CHECK (pssize_val == (pssize) 0x00F0000000000000);
			BOOST_CHECK (PSSIZE_FROM_BE (pssize_val) == (pssize) 0x000000000000F000);
			BOOST_CHECK (PSSIZE_TO_LE (pssize_val) == (pssize) 0x00F0000000000000);
			BOOST_CHECK (PSSIZE_FROM_LE (pssize_val) == (pssize) 0x00F0000000000000);
		} else {
		...
		pint64 pint64_val = PINT64_TO_LE (0xFFFFFFFFFFFFFF80);
		BOOST_CHECK (pint64_val == (pint64) 0x80FFFFFFFFFFFFFF);
		BOOST_CHECK (PINT64_FROM_LE (pint64_val) == (pint64) 0xFFFFFFFFFFFFFF80);
		BOOST_CHECK (PINT64_TO_BE (pint64_val) == (pint64) 0x80FFFFFFFFFFFFFF);
		BOOST_CHECK (PINT64_FROM_BE (pint64_val) == (pint64) 0x80FFFFFFFFFFFFFF);

		puint64 puint64_val = PUINT64_TO_LE (0x0000000000000080U);
		BOOST_CHECK (puint64_val == (puint64) 0x8000000000000000U);
		BOOST_CHECK (PUINT64_FROM_LE (puint64_val) == (puint64) 0x0000000000000080U);
		BOOST_CHECK (PUINT64_TO_BE (puint64_val) == (puint64) 0x8000000000000000U);
		BOOST_CHECK (PUINT64_FROM_BE (puint64_val) == (puint64) 0x8000000000000000U);
		...
		if (PLIB_SIZEOF_LONG == 8) {
			plong plong_val = PLONG_TO_LE (0xFFFFFFFFFFFFF800);
			BOOST_CHECK (plong_val == (plong) 0x00F8FFFFFFFFFFFF);
			BOOST_CHECK (PLONG_FROM_LE (plong_val) == (plong) 0xFFFFFFFFFFFFF800);
			BOOST_CHECK (PLONG_TO_BE (plong_val) == (plong) 0x00F8FFFFFFFFFFFF);
			BOOST_CHECK (PLONG_FROM_BE (plong_val) == (plong) 0x00F8FFFFFFFFFFFF);

			pulong pulong_val = PULONG_TO_LE (0x0000000000000800U);
			BOOST_CHECK (pulong_val == (pulong) 0x0008000000000000U);
			BOOST_CHECK (PULONG_FROM_LE (pulong_val) == (pulong) 0x0000000000000800U);
			BOOST_CHECK (PULONG_TO_BE (pulong_val) == (pulong) 0x0008000000000000U);
			BOOST_CHECK (PULONG_FROM_BE (pulong_val) == (pulong) 0x0008000000000000U);
		} else {
		...
		if (PLIB_SIZEOF_SIZE_T == 8) {
			psize psize_val = PSIZE_TO_LE (0x0000000000001000U);
			BOOST_CHECK (psize_val == (psize) 0x0010000000000000U);
			BOOST_CHECK (PSIZE_FROM_LE (psize_val) == (psize) 0x0000000000001000U);
			BOOST_CHECK (PSIZE_TO_BE (psize_val) == (psize) 0x0010000000000000U);
			BOOST_CHECK (PSIZE_FROM_BE (psize_val) == (psize) 0x0010000000000000U);

			pssize pssize_val = PSSIZE_TO_LE (0x000000000000F000);
			BOOST_CHECK (pssize_val == (pssize) 0x00F0000000000000);
			BOOST_CHECK (PSSIZE_FROM_LE (pssize_val) == (pssize) 0x000000000000F000);
			BOOST_CHECK (PSSIZE_TO_BE (pssize_val) == (pssize) 0x00F0000000000000);
			BOOST_CHECK (PSSIZE_FROM_BE (pssize_val) == (pssize) 0x00F0000000000000);
		} else {
	...
	puint64 puint64_val = PUINT64_SWAP_BYTES (0x0000000000000080U);
	BOOST_CHECK (puint64_val == (puint64) 0x8000000000000000U);
	BOOST_CHECK (PUINT64_SWAP_BYTES (puint64_val) == (puint64) 0x0000000000000080U);
