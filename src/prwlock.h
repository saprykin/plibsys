 ...
 * than writer ones.
 *
 * A reader enters a critical section with p_rwlock_reader_lock() and exits
 * with p_rwlock_unlock(). A writer enters the critical section with
 * p_rwlock_writer_lock() and exits with p_rwlock_unlock(). The try-to-lock
...
/**
 * @brief Releases a locked read-write lock.
 ...
 *
 * If the @a lock was previously locked then it becomes unlocked.
 *
 * It's implementation dependent whether only the same thread can lock and
 * unlock the same read-write lock.
 */
P_LIB_API pboolean	p_rwlock_unlock		(PRWLock *lock);
